...
def load_dicom(fp: Union[str, "os.PathLike[AnyStr]", BinaryIO]):
    """Cargar archivo DICOM en un hilo separado"""
    global ds
    ds = dicom.dcmread(fp)

def dcmread_image(fp: Union[str, "os.PathLike[AnyStr]", BinaryIO], view: str, index: Optional[np.uint] = None) -> np.ndarray:
    """Leer matriz de píxeles desde el archivo DICOM DBT"""
    # Cargar archivo DICOM en un hilo separado
    thread = Thread(target=load_dicom, args=(fp,))
    thread.start()
    thread.join()

    # Una vez cargado el archivo DICOM, procesarlo
